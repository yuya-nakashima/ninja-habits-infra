...
## 残アクション
- **prod 値の差し替え**: `config.prod.apiAllowedOrigin`（現 `https://example.com`）と Auth callback/logout URL を本番 Web オリジンへ（prod デプロイ前に対応）。
- **WAF**: ALB へのレートリミット・IP フィルタ（T-019 残）。
- **prod environment gate**: `release-api.yml` の prod 向け job に GitHub Environment の承認ゲートを付与することを検討。
