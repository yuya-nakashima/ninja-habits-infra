...
デプロイ直後・リリース後に以下を順番に確認する。全項目が通れば prod 正常とみなす。

### 1. API ヘルスチェック

```bash
curl -s https://api.ninja-habits.com/health
# → {"status":"ok"}  (DB 接続含む)
```
...
```bash
curl -si -X OPTIONS https://api.ninja-habits.com/v1/today \
  -H "Origin: https://ninja-habits.com" \
  -H "Access-Control-Request-Method: GET" | grep -i access-control
# → Access-Control-Allow-Origin: https://ninja-habits.com が含まれること
```

### 3. Web 表示

ブラウザで `https://ninja-habits.com/` を開く。
- NINJA HABITS ログイン画面が表示されること
- コンソールエラーがないこと（DevTools で確認）

### 4. ログイン

Cognito Hosted UI (`https://ninja-habits-prod.auth.ap-northeast-1.amazoncognito.com`) 経由でログイン。
- ログイン後 `https://ninja-habits.com/` にリダイレクトされること
- 今日の画面が表示されること（初回は空状態）

### 5. 初期データ作成（UI 操作）

| 操作 | 確認点 |
|------|-------|
| Goal を 1 件追加 | 画面に表示される |
| Habit グループ + 項目を 1 件追加 | 画面に表示される |
| Wish カテゴリ + 項目を 1 件追加 | 画面に表示される |

### 6. 振り返り保存

ホーム画面の振り返りフォームに任意テキストを入力して保存。
- エラーなく保存できること
- ストリーク表示が更新されること

### 7. 再ログイン後のデータ確認

ログアウト → 再ログインして、追加したデータが残っていることを確認。
...
aws logs filter-log-events \
  --log-group-name aws-waf-logs-ninja-habits-prod-api \
  --start-time $(date -d '10 minutes ago' +%s000 2>/dev/null || date -v -10M +%s000) \
  --filter-pattern '{ $.action = "BLOCK" }' \
  --region ap-northeast-1 | jq '.events[].message | fromjson | {action, httpRequest.uri, terminatingRuleId}'
# → BLOCK が 0 件（または意図した件数）であること
```
...
prod RDS は `backupRetentionDays=14`（自動スナップショット 14 日間）と `deletionProtection=true` を設定済み。

**手動スナップショット取得（リリース前など）:**

```bash
aws rds create-db-snapshot \
  --db-instance-identifier <instance-id> \
  --db-snapshot-identifier ninja-habits-prod-manual-$(date +%Y%m%d) \
  --region ap-northeast-1
# instance-id は RDS コンソールまたは database-stack の出力から確認
```
...
aws rds describe-db-snapshots \
  --query 'DBSnapshots[?contains(DBSnapshotIdentifier,`ninja-habits-prod`)][DBSnapshotIdentifier,SnapshotCreateTime,Status]' \
  --output table --region ap-northeast-1
```

### RDS 復旧手順

自動スナップショットから特定時点への復元:

1. RDS コンソール → 対象スナップショットを選択 → **Restore snapshot**
2. 新しい DB インスタンス名を指定（既存インスタンスは上書きされない）
3. 復元後、`config.ts` の `databaseName` または DATABASE_URL を新インスタンスに向ける
4. SSM パラメータ `/ninja-habits/prod/db/endpoint` を更新し、インスタンスを再起動
...
```bash
# 1. 前のバージョンの S3 キーを確認
aws s3 ls s3://ninja-habits-api-artifacts-prod-720623131603/api/ \
  --region ap-northeast-1 | sort | tail -5

# 2. 前のバージョンに promote（migration の巻き戻しは不要な場合のみ）
npm run set-api-artifact:prod -- promote api/ninja-habits-api-<旧LABEL>.tgz

# 3. instance refresh で前バージョンのインスタンスに置き換え
npm run refresh-api:prod
```

> **注意**: DB migration の巻き戻しは自動では行われない。
> 後方非互換な migration を適用してロールバックする場合は手動で down migration を実行すること。
