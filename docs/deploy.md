...
通常リリース(§6.1)は「稼働中の SSM 管理インスタンス」を前提にするが、初回はそれが存在しない。
鶏卵問題（artifact-key が無いと user-data が失敗 / migration 前に API が起動 / migration 対象インスタンスが未作成）を避けるため、初回は以下の順で行う:

1. S3 成果物バケットを用意（api-stack に含めるなら先に api-stack の該当リソースのみでも可）
2. **新 artifact を S3 アップロード**し、**SSM `artifact-key` と各種 SSM パラメータ／DB シークレット参照を先に用意**（user-data が起動時に解決できる状態にする）
3. **database-stack をデプロイ**（RDS 作成）。endpoint/dbname を SSM へ投入
4. **api-stack をデプロイ**（ASG 起動 → user-data が artifact を pull → systemd 起動）。
   - この時点では DB 未マイグレートのため `/v1/*` は失敗するが、`/health` は `SELECT 1` のみで通り ELB ヘルスチェックは PASS する（インスタンスは InService になる）。
5. 対象インスタンスが **SSM online** になるのを待つ
6. **新 artifact で migration を実行**（§5 の SSM Run Command）
7. **ALB 経由で API 動作確認**（`/v1/me` 等が 200 になること）
- 以降は §6.1 の通常リリースに移行する。

> 留意: 初回はごく短時間「未マイグレート API が公開」される。dev では許容。prod 公開時は (a) 先に最小1台で migration → その後スケールアウト、または (b) ALB リスナーを後段で有効化、で露出を避ける（T-019 と合わせて検討）。
